#!/usr/bin/env bash
# ============================================================================
# setup-cert.sh — provision AgentDeck's TLS certificate from a local mkcert
# CA and install trust everywhere the environment needs it.
#
...
#   6. restarts the agentdeck systemd user service when present
#   7. prints iOS trust instructions (or serve+QR with --ios)
#
# Usage:
#   scripts/setup-cert.sh [--ios]
# ============================================================================
...
# --------------------------------------------------------------------------
bold "[4/6] issuing certificate (data/cert.pem, data/key.pem)"
mkdir -p "$ROOT/data"
CAROOT_VAL="$(mkcert -CAROOT)"
mkcert -cert-file "$ROOT/data/cert.pem" -key-file "$ROOT/data/key.pem" \
    "${SAN[@]}" >/dev/null 2>&1 \
...
if [[ $IS_WSL -eq 1 && -n "$PWSH" ]]; then
    ...
    [[ -n "$win_user" ]] || { warn "could not detect the Windows user — import manually:"; }
    if [[ -n "${win_user:-}" ]]; then
        ca_win="/mnt/c/Users/$win_user/agentdeck-mkcert-rootCA.cer"
        cp "$CAROOT_VAL/rootCA.pem" "$ca_win"
        ok "CA exported to C:\\Users\\$win_user\\agentdeck-mkcert-rootCA.cer"
        ...
        if [[ "${already:-0}" -ge 1 ]]; then
            ...
            "$PWSH" -NoProfile -Command \
                "Start-Process powershell -Verb RunAs -Wait -ArgumentList '-NoProfile','-Command','Import-Certificate -FilePath C:\\Users\\$win_user\\agentdeck-mkcert-rootCA.cer -CertStoreLocation Cert:\LocalMachine\Root'" \
                2>/dev/null || true
            verify="$($PWSH -NoProfile -Command \
                "(Get-ChildItem Cert:\LocalMachine\Root | Where-Object Subject -like '*mkcert*').Count" 2>/dev/null | tr -d '\r\n ' || echo 0)"
            [[ "${verify:-0}" -ge 1 ]] && ok "CA imported into Windows trust store" \
                                     || warn "import not confirmed — run manually as Admin: Import-Certificate -FilePath C:\\Users\\$win_user\\agentdeck-mkcert-rootCA.cer -CertStoreLocation Cert:\LocalMachine\Root"
        fi
...
# --------------------------------------------------------------------------
bold "[6/6] restarting AgentDeck"
if systemctl --user is-active --quiet agentdeck 2>/dev/null; then
    systemctl --user restart agentdeck
    ok "systemd user service restarted"
elif pgrep -f "bin/agentdeck" &>/dev/null; then
    warn "agentdeck is running outside systemd — restart it manually to pick up the new cert"
else
    ok "agentdeck not running — cert will be used on next start"
fi
