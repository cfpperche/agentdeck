// Package share diagnoses whether a phone can open this AgentDeck instance
...
// DesiredNames is localhost + agentdeck.local + current phone-usable IPs.
func DesiredNames() []string {
	names := []string{"localhost", "agentdeck.local"}
	seen := map[string]bool{"localhost": true, "agentdeck.local": true}
...
func ReachableIPv4() []string {
	...
	for _, s := range out {
		if isTailnet(net.ParseIP(s)) {
			...
			// (another node) shows up on eth* and is not where agentdeck listens.
...
func certInfo(dataDir string) (sans []string, issuer string) {
	...
	pair, err := tls.LoadX509KeyPair(filepath.Join(dataDir, certFile), filepath.Join(dataDir, keyFile))
