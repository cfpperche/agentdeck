...
func tryOpenWindowsPorts(ports ...int) {
	...
	script := fmt.Sprintf(
		`$n='AgentDeck LAN'; if (-not (Get-NetFirewallRule -DisplayName $n -ErrorAction SilentlyContinue)) { New-NetFirewallRule -DisplayName $n -Direction Inbound -Action Allow -Protocol TCP -LocalPort %s | Out-Null }`,
		list,
	)
