...
func EnsureTrustHTTP() string {
	...
	mux.HandleFunc("GET /rootCA.pem", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Disposition", "attachment; filename=agentdeck-rootCA.pem")
		_, _ = w.Write(ca)
	})
	mux.HandleFunc("GET /rootCA.cer", func(w http.ResponseWriter, r *http.Request) {
		der := pemToDER(ca)
		w.Header().Set("Content-Type", "application/x-x509-ca-cert")
		w.Header().Set("Content-Disposition", "attachment; filename=agentdeck-rootCA.cer")
		_, _ = w.Write(der)
	})
	mux.HandleFunc("GET /agentdeck-ca.mobileconfig", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/x-apple-aspen-config")
		w.Header().Set("Content-Disposition", "attachment; filename=agentdeck-ca.mobileconfig")
	...
	tryOpenWindowsPorts(8444, trustPort)
	return port
}
...
<main data-os="` + os + `" id="wiz">
  ...
  <section id="s1">
    <h1>Install trust</h1>
    <p class="lead ios">Safari will say the profile is not signed. That’s normal for a local CA — tap Install anyway.</p>
  ...
  <section id="s2" class="hidden">
    ...
    <div class="card ios">
      ...
      tap the AgentDeck profile → Install
    </div>
    <div class="card ios">
      <b>Settings → General → About → Certificate Trust Settings</b>
      enable <b>AgentDeck mkcert</b>
    </div>
...
func openBtn(next string) string {
	...
	return `<a class="btn btn-pri" href="` + next + `">Open AgentDeck</a>`
}
...
<key>PayloadCertificateFileName</key><string>agentdeck-mkcert.cer</string>
